...
class Board:
    ...
    def rockFail(self, x, y):
        ...
        if self.board[x][y].num > 0:
            for i, j in filter(
                lambda edge: edge[0] == (x, y) or edge[1] == (x, y), self.path.edges
            ):
    ...
    def createLayout(self):
        ...
        self.addNetwork(
            self.rock[0][0], self.rock[0][1], board[self.rock[0][0], self.rock[0][1]]
        )
        self.addNetwork(
            self.rock[1][0], self.rock[1][1], board[self.rock[1][0], self.rock[1][1]]
        )
        return board
    ...
    def removeNetwork(self, x, y, card):
        for i, j in filter(
            lambda x: x[0] == (10, 7) or x[1] == (10, 7), self.path.edges
        ):
            self.path.remove_edge(i, j)
            self.path.nodes[(x, y)]["active"] = False
    ...
    def activeHasPath(self, path, source, target):
        ...
        active_nodes = [
            n for n, attr in path.nodes(data=True) if attr.get("active", True)
        ]
    ...
    def showBoard(self):
        ...
        for r in range(rows):
            ...
            cells_lines = [
                ["".join(row) for row in cell.map] for cell in self.board[r, :]
            ]

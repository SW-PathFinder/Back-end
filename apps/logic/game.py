        ...
        if player == self.currentPlayer:
            ...
            match action["type"]:
                case (
                    ...
                    # 경로 추가
                    result = self.board.addCard(
                        action["data"]["card"], action["data"]["x"], action["data"]["y"]
                    )
                    print("Result : ", result)
                    if result == True:
                        # 경로 추가 성공
                ...
                case (
                    "saboteur"
                ):  # action = {"type":"saboteur","data":{"target":str,"cardType":str}}
                    # 사보타주 행동
                    result = self.players[action["target"]].setLimit(
                        action["data"]["cardType"]
                    )
                    if result == True:
                        # 사보타주 성공
                        self.tasks.append(
                ...
                case (
                    "repair"
                ):  # action = {"type":"repair","data":{"target":str,"cardType":str}}
                    result = self.players[action["target"]].repairLimit(
                        action["data"]["cardType"]
                    )
                    if result == True:
                        # 수리 성공
                ...
                case "viewMap":  # action = {"type":"viewMap","data":{"x":x,"y":y}}
                    # 맵 보기
                    result = self.board.viewMap(
                        action["data"]["x"], action["data"]["y"]
                    )
                    if result[0] == True:
                        # 맵 보기 성공
                        self.tasks.append(
                            {"type": "viewMap", "data": {"data": result[1]}}
                        )
                        self._nextTrun()
                    else:
                        self.tasks.append({"type": "error", "data": result[1]})

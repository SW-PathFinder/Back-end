...
##### 인게임 #######
    ...
    def action(self, player, action):
        ...
        if action["type"] == "hand":
            ...
            # print(data)
            return {"player":player,"target":player,"type":"hand","data":data}

        elif action["type"] == "playerState":
            data = self.getPlayerState(player)
            return {"player":player,"target":player,"type":"playerState","data":data}
    ...
    def getplayerState(self, player):
        return {
            ...
            "board": [{"x": x, "y": y, "cardId": self.board.board[x, y].num if (-2,-4,-6) not in self.card.board.board[x,y].num else -8 , "reverse": self.board.board[x, y].flip} for x in range(22) for y in range(22) if self.board.board[x, y].num != 0],
